load('Mystery dataset 2 OTU tables based on common features.RData')
library(limma)
...
result_species_family_mystery_RF<-RF_CV(otu_normalized,nrow(otu_normalized))
result_species_family_mystery_SVM<-SVM_CV(otu_normalized,nrow(otu_normalized))
result_species_family_mystery_LDA<-LDA_CV(otu_normalized,nrow(otu_normalized))
...
result_species_order_mystery_RF<-RF_CV(otu_normalized,nrow(otu_normalized))
result_species_order_mystery_SVM<-SVM_CV(otu_normalized,nrow(otu_normalized))
result_species_order_mystery_LDA<-LDA_CV(otu_normalized,nrow(otu_normalized))
...
result_family_order_mystery_RF<-RF_CV(otu_normalized,nrow(otu_normalized))
result_family_order_mystery_SVM<-SVM_CV(otu_normalized,nrow(otu_normalized))
result_family_order_mystery_LDA<-LDA_CV(otu_normalized,nrow(otu_normalized))
...
result_common_combine_mystery_RF<-RF_CV(otu_normalized,nrow(otu_normalized))
result_common_combine_mystery_SVM<-SVM_CV(otu_normalized,nrow(otu_normalized))
result_common_combine_mystery_LDA<-LDA_CV(otu_normalized,nrow(otu_normalized))

## species/family/order
## species 
otu_normalized<-normalization_f(otu_common_species_mystery)
result_species_mystery_RF<-RF_CV(otu_normalized,nrow(otu_normalized))
result_species_mystery_SVM<-SVM_CV(otu_normalized,nrow(otu_normalized))
result_species_mystery_LDA<-LDA_CV(otu_normalized,nrow(otu_normalized))
## family
otu_normalized<-normalization_f(otu_common_family_mystery)
result_family_mystery_RF<-RF_CV(otu_normalized,nrow(otu_normalized))
result_family_mystery_SVM<-SVM_CV(otu_normalized,nrow(otu_normalized))
result_family_mystery_LDA<-LDA_CV(otu_normalized,nrow(otu_normalized))
## order
otu_normalized<-normalization_f(otu_common_order_mystery)
result_order_mystery_RF<-RF_CV(otu_normalized,nrow(otu_normalized))
result_order_mystery_SVM<-SVM_CV(otu_normalized,nrow(otu_normalized))
result_order_mystery_LDA<-LDA_CV(otu_normalized,nrow(otu_normalized))

RF_importance<-result_species_order_mystery_RF$plot_importance_score
tiff(paste0( "RF_importance_mystery",".tif"), height=1400, width=1400, res=230, units="px");print(RF_importance);dev.off()
...
#### sequencing depth ####
otu_common_species_order_mystery_0<-otu_common_species_order_mystery[,which(colSums(otu_common_species_order_mystery)!=0)]
otu_count<-as.data.frame(cbind(apply(otu_common_species_order_mystery_0,2,sum),substr(colnames(otu_common_species_order_mystery_0),1,3)))
...
error_city_dup$city<-substr(error_city_dup$city,1,3)
...
tiff(paste0( "plot_sequencing_depth_mystery",".tif"), height=1400, width=1400, res=230, units="px");print(plot_sequencing_depth);dev.off()

#### data for the following analyses ####
save(list=c('otu_common_species_order_mystery'),
     file='otu_common_species_order.RData')
